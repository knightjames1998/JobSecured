...
(function () {
  ...
  function calc() {
    ...
    var yearEl = document.getElementById("netYearly");
    if (net > 0) {
      netEl.textContent = fmt(net);
      yearEl.textContent = fmt(net * 12);
      yearEl.parentElement.innerHTML =
        "That's <strong id=\"netYearly\">" + fmt(net * 12) +
        "</strong> a year — from calls you're already getting.";
    } else {
      netEl.textContent = fmt(0);
      yearEl.parentElement.innerHTML =
        "At this volume, start with <strong>After‑Hours Guard at $199</strong> and grow from there.";
    }
  }
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (form.action.indexOf("YOUR_FORM_ID") !== -1) {
        e.preventDefault();
        alert("Form isn't wired up yet — call or text (210) 500-0096 and we'll get right back to you.");
        return;
      }
      e.preventDefault();
      ...
      fetch(form.action, {
        ...
        .then(function (r) {
          if (r.ok) {
            form.outerHTML =
              '<div class="form-success">Got it. James will call you back the same day — usually within the hour.</div>';
          } else {
            throw new Error();
          }
        })
